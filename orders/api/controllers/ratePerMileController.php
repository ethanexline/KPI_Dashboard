<?php
require_once("./objects/rate_per_mile.php"); //Order Count  object

header('Access-Control-Allow-Origin: *');
header('Content-Type: application/json');

class ratePerMileController
{
    function get_rate_per_mile($company = FALSE, $division = FALSE, $terminal = False, $start_date = False, $end_date = False)
    {
        ...
        $sql = "
                ...
                SELECT sum({REDACTED})/sum({REDACTED}) 'amount', 
                (select avg({REDACTED}) 
                from (SELECT sum({REDACTED})/sum({REDACTED}) 'rpm' from {REDACTED} join {REDACTED}.{REDACTED} on {REDACTED} = date
                join {REDACTED} dd on {REDACTED}.{REDACTED} = dd.{REDACTED}
                where {REDACTED} between " . $startDate . " and " . $endDate . ") e) 'average'
        ...
        if($results)
        {
            ...
            {
                $week_object = new rate_per_mile($revenue -> amount, $revenue -> average);
    ...
    function post()
    {
        ...
        if(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and empty($sort->startDate) and empty($sort->endDate))
        {
            ...
            echo json_encode($this -> get_rate_per_mile($companies, $divisions, $terminals));
        ...
        elseif(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and !empty($sort->startDate) and empty($sort->endDate))
        {
            ...
            echo json_encode($this -> get_rate_per_mile($companies, $divisions, $terminals, $startDate));
        ...
        elseif(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and !empty($sort->startDate) and !empty($sort->endDate))
        {
            ...
            echo json_encode($this -> get_rate_per_mile($companies, $divisions, $terminals, $startDate, $endDate));
