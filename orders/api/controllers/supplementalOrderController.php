<?php
require_once("./objects/week_revenue.php"); //Order Count  object

header('Access-Control-Allow-Origin: *');
header('Content-Type: application/json');

class supplementalRevenueController
{
    function get_supplemental_revenue($company = FALSE, $division = FALSE, $terminal = False, $start_date = False, $end_date = False)
    {
        ...
        $sql = "DECLARE @start_year int, @start_week int, @end_year int, @end_week int, @start_date date, @end_date date
                ...
                SELECT {REDACTED}, {REDACTED}, sum({REDACTED}) 'amount' from {REDACTED}
        ...
        $sql2 = "DECLARE @start_year int, @start_week int, @end_year int, @end_week int, @start_date date, @end_date date
                SET @end_year = (SELECT {REDACTED} from {REDACTED}.{REDACTED} dd where dd.date = cast(dateadd(day, 0, getdate()) as date))
                SET @end_week = (SELECT {REDACTED} from {REDACTED}.{REDACTED} dd where dd.date = cast(dateadd(day, 0, getdate()) as date))
                ...
                join {REDACTED} dd on {REDACTED}.{REDACTED} = dd.{REDACTED}
                where {REDACTED} between " . $startDate . " and " . $endDate . "
                and {REDACTED} in ('XLDTES', 'XTRLRN' , 'XCATR', 'XTRART', 'XDPLTR') 
                " . $company_sql . "
                group by {REDACTED}, {REDACTED}
                order by {REDACTED}, {REDACTED}
        ";
        ...
        if($results)
        {
            ...
            {
                $week_object = new week_revenue($revenue -> amount, $revenue -> {REDACTED}, $revenue -> Year);
                array_push($weeks_revenue, $week_object);
            }
            while($revenue = sqlsrv_fetch_object($results2))
            {
                $week_object2 = new week_revenue($revenue -> amount, $revenue -> {REDACTED}, $revenue -> Year);
                array_push($weeks_revenue2, $week_object2);
            }
        }
        
        return array($weeks_revenue, $weeks_revenue2);
    }
    ...
    function post()
    {
        ...
        if(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and empty($sort->startDate) and empty($sort->endDate))
        {
            ...
            echo json_encode($this -> get_supplemental_revenue($companies, $divisions, $terminals));
        ...
        elseif(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and !empty($sort->startDate) and empty($sort->endDate))
        {
            ...
            echo json_encode($this -> get_supplemental_revenue($companies, $divisions, $terminals, $startDate));
        ...
        elseif(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and !empty($sort->startDate) and !empty($sort->endDate))
        {
            ...
            echo json_encode($this -> get_supplemental_revenue($companies, $divisions, $terminals, $startDate, $endDate));
