<?php
require_once("./objects/loaded_miles.php"); //Order Count  object

header('Access-Control-Allow-Origin: *');
header('Content-Type: application/json');

class weeklyLoadedMilesController
{
    function get_weekly_loaded_miles($company = FALSE, $division = FALSE, $terminal = False, $start_date = False, $end_date = False)
    {
        ...
        SELECT sum({REDACTED}) 'amount', {REDACTED}, {REDACTED} 
        ...
        if($results)
        {
            ...
            {
                $week_object = new loaded_miles($revenue -> amount, $revenue -> {REDACTED}, $revenue -> Year);
   ...
   function post()
   {
       ...
       if(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and empty($sort->startDate) and empty($sort->endDate))
       {
           ...
           echo json_encode($this -> get_weekly_loaded_miles($companies, $divisions, $terminals));
       ...
       elseif(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and !empty($sort->startDate) and empty($sort->endDate))
       {
           ...
           echo json_encode($this -> get_weekly_loaded_miles($companies, $divisions, $terminals, $startDate));
       ...
       elseif(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and !empty($sort->startDate) and !empty($sort->endDate))
       {
           ...
           echo json_encode($this -> get_weekly_loaded_miles($companies, $divisions, $terminals, $startDate, $endDate));
