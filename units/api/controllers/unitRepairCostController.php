 <?php
require_once("./objects/unit_repair_cost.php"); //Order Count  object

header('Access-Control-Allow-Origin: *');
header('Content-Type: application/json');

class unitRepairCostController
{
    function get_repair_order_cost($company = FALSE, $division = FALSE, $terminal = False, $start_date = False, $end_date = False)
    {
        $database = new database();
        $connection = $database -> connection();
        $repair_cost_all = array();
        ...
        $sql = "DECLARE @start_year int, @start_week int, @end_year int, @end_week int, @start_date date, @end_date date
                ...
                SELECT {REDACTED}, {REDACTED}, sum({REDACTED}) 'amount' 
                from {REDACTED} umd
                ...
                where utr.{REDACTED} between " . $startDate . " and " . $endDate . " and {REDACTED} = 'RO'
                " . $company_sql . "
        ...
        if($results)
        {
            ...
            {
                $repair_cost = new unit_repair_cost($status -> amount, $status -> year, $status -> {REDACTED});
                array_push($repair_cost_all, $repair_cost);
            }
        }
        
        return $repair_cost_all;
    }
    ...
    function post()
    {
        ...
        if(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and empty($sort->startDate) and empty($sort->endDate))
        {
            ...
            echo json_encode($this -> get_repair_order_cost($companies, $divisions, $terminals));
        }
        ...
        elseif(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and !empty($sort->startDate) and empty($sort->endDate))
        {
            ...
            echo json_encode($this -> get_repair_order_cost($companies, $divisions, $terminals, $startDate));
        }
        ...
        elseif(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and !empty($sort->startDate) and !empty($sort->endDate))
        {
            ...
            echo json_encode($this -> get_repair_order_cost($companies, $divisions, $terminals, $startDate, $endDate));
        }
    }
}
