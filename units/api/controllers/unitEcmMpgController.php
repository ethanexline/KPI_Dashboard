...
class unitEcmMpgController
{
    function get_weekly_ecm_mpg($company = FALSE, $division = FALSE, $terminal = False, $start_date = False, $end_date = False)
    {
        ...
        $sql = "DECLARE @start_year int, @start_week int, @end_year int, @end_week int, @start_date date, @end_date date
                ...
                select dd2.{REDACTED}, {REDACTED}, sum({REDACTED}) 'distance', case when sum({REDACTED}) = 0 then 1 else sum({REDACTED}) end 'gallons'
                from {REDACTED} udp 
                ...
                where udp.date between " . $startDate . " and " . $endDate . " and {REDACTED} < 2000
                " . $company_sql . " 
        ...
        if($results)
        {
            ...
            {
                $unit_status_object = new unit_ecm_mpg($status -> {REDACTED}, $status -> {REDACTED}, $status -> {REDACTED}, $status -> gallons);
    ...
    function post()
    {
        ...
        if(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and empty($sort->startDate) and empty($sort->endDate))
        {
            ...
            echo json_encode($this -> get_weekly_ecm_mpg($companies, $divisions, $terminals));
        ...
        elseif(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and !empty($sort->startDate) and empty($sort->endDate))
        {
            ...
            echo json_encode($this -> get_weekly_ecm_mpg($companies, $divisions, $terminals, $startDate));
        ...
        elseif(!empty($sort->companies) and !empty($sort->divisions) and !empty($sort->terminals) and !empty($sort->startDate) and !empty($sort->endDate))
        {
            ...
            echo json_encode($this -> get_weekly_ecm_mpg($companies, $divisions, $terminals, $startDate, $endDate));
        }
    }
}
