...
{
    function get_transaction_summary($sort)
    {
        $parameters = array();
        ...
        if($sort != null)
        {
            ...
            $sql = "
            SELECT SUM([{REDACTED}]) 'total_gals'
            ,SUM([{REDACTED}]) 'DEF_gals'
            ,SUM([{REDACTED}]) 'total_cost'
            ,SUM([{REDACTED}])'DEF_cost'
            ,SUM([{REDACTED}]) 'rf_gals'
            ,SUM([{REDACTED}]) 'rf_cost'
            ,SUM([{REDACTED}]) 'tot_rebate'
            ,SUM([fees]) 'fees'
            ,COUNT(*) 'stops'
            ,SUM(({REDACTED} - {REDACTED}) + {REDACTED}) 'reb_gallons'
            ...
            if ($results) {
                $transactionSummary = sqlsrv_fetch_object($results);

                //Setup the return object.
                $return_object = new transactionSummary($transactionSummary -> total_gals, $transactionSummary -> DEF_gals, $transactionSummary -> total_cost,
                $transactionSummary -> DEF_cost, $transactionSummary -> rf_gals, $transactionSummary -> rf_cost, $transactionSummary -> tot_rebate,
                $transactionSummary -> fees, $transactionSummary -> stops, $transactionSummary -> reb_gallons);
            }
    ...
    function post()
    {
        ...
        if(!empty($_POST['sort']))
        {
            try
            {
                ...
                if($validator === true)
                {
                    $transactions = $this -> get_transaction_summary($sort);
                    return $transactions;
                }
                else
                {
