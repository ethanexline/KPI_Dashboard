...
class CostByTerminalController
{
    function get_cost_by_terminal($sort)
    {
        $parameters = array();
        ...
        if($sort != null)
        {
            ...
            $sql = "
            SELECT case when {REDACTED} is null then 'None' else {REDACTED} end 'terminal', 
            sum({REDACTED}) 'cost'
            ...
            if ($results) {
                while($terminal = sqlsrv_fetch_object($results))
                    array_push($terminals, array($terminal -> terminal, $terminal -> cost));
            }
    ...
    function post()
    {
        ...
        if(!empty($_POST['sort']))
        {
            try
            {
                ...
                if($validator === true)
                {
                    $transactions = $this -> get_cost_by_terminal($sort);
                    return $transactions;
                }
                else
                {
