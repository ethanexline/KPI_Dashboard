...
class PPGByDayController
{
    function get_PPG_by_day($sort)
    {
        $parameters = array();
        ...
        if($sort != null)
        {
            ...
            case when sum({REDACTED}) = 0 then 0 else (sum(case when {REDACTED} = 0 then 0 else ({REDACTED} - case when {REDACTED} = 0 then 0 else 
                (1 - (case when (({REDACTED} - {REDACTED}) + {REDACTED}) = 0 then 0 else ((({REDACTED} - {REDACTED}) / 
                (({REDACTED} - {REDACTED}) + {REDACTED}))) end)) * {REDACTED} end) end)) / sum({REDACTED}) end 'reefer_ppg',

            case when sum({REDACTED}) = 0 then 0 else sum({REDACTED}) / sum({REDACTED}) end 'DEF_ppg'

            FROM [{REDACTED}].[{REDACTED}].[{REDACTED}] ft
            ...
            if ($results) {
                while($type = sqlsrv_fetch_object($results))
                    array_push($types, array($type -> date, $type -> tractor_ppg, $type -> reefer_ppg, $type -> DEF_ppg));
            }
    ...
    function post()
    {
        ...
        if(!empty($_POST['sort']))
        {
            try
            {
                ...
                if($validator === true)
                {
                    $transactions = $this -> get_PPG_by_day($sort);
                    return $transactions;
                }
                else
                {
