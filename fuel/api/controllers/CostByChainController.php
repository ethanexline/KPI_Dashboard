...
class CostByChainController
{
    function get_cost_by_chain($sort)
    {
        $parameters = array();
        ...
        if($sort != null)
        {
            ...
            $sql = "
            SELECT case when fc.{REDACTED} is null then 'Undetermined' else fc.{REDACTED} end 'chain', 
            sum({REDACTED}) 'cost'
            ...
            if ($results) {
                while($chain = sqlsrv_fetch_object($results))
                    array_push($chains, array($chain -> chain, $chain -> cost));
            }
    ...
    function post()
    {
        ...
        if(!empty($_POST['sort']))
        {
            try
            {
                ...
                if($validator === true)
                {
                    $transactions = $this -> get_cost_by_chain($sort);
                    return $transactions;
                }
                else
                {
