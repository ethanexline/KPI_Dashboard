...
class CostByFuelTypeController
{
    function get_cost_by_fuel_type($sort)
    {
        $parameters = array();
        ...
        if($sort != null)
        {
            ...
            $sql = "
            SELECT 
            sum(case when {REDACTED} - {REDACTED} = 0 then 0 else (({REDACTED} - {REDACTED}) - case when {REDACTED} = 0 then 0 else 
                ((({REDACTED} - {REDACTED}) / (({REDACTED} - {REDACTED}) + {REDACTED})) * {REDACTED}) end) end) 'tractor_cost',

            sum(case when {REDACTED} = 0 then 0 else ({REDACTED} - case when {REDACTED} = 0 then 0 else (1 - (case when (({REDACTED} - {REDACTED}) + 
            {REDACTED}) = 0 then 0 else ((({REDACTED} - {REDACTED}) / 
                (({REDACTED} - {REDACTED}) + {REDACTED}))) end)) * {REDACTED} end) end) 'reefer_cost',

            sum({REDACTED}) 'DEF_cost',

            sum({REDACTED} + {REDACTED}) 'cash_advance',

            sum({REDACTED} + {REDACTED} + {REDACTED} + {REDACTED} + {REDACTED}) 'all_else'
            ...
            if ($results) {
                while($type = sqlsrv_fetch_object($results))
                    array_push($types, array($type -> tractor_cost, $type -> reefer_cost, $type -> DEF_cost, $type -> cash_advance, $type -> all_else));
            }
    ...
    function post()
    {
        ...
        if(!empty($_POST['sort']))
        {
            try
            {
                ...
                if($validator === true)
                {
                    $transactions = $this -> get_cost_by_fuel_type($sort);
                    return $transactions;
                }
                else
                {
